 ...
 * (c) 2012 Attila Szasz, tilusnet.com
 */
public class MercatorMap {
  ...
  /** Horizontal offset of this map, in pixels. */
  private float mapScreenOffsetX;
  /** Vertical offset of this map, in pixels. */
  private float mapScreenOffsetY;
  ...
  public MercatorMap(float mapScreenWidth, float mapScreenHeight) {
    this(0.0, 0.0, mapScreenWidth, mapScreenHeight, DEFAULT_TOP_LATITUDE, DEFAULT_BOTTOM_LATITUDE, DEFAULT_LEFT_LONGITUDE, DEFAULT_RIGHT_LONGITUDE);
  }
  ...
  public MercatorMap(float mapScreenWidth, float mapScreenHeight, float topLatitude, float bottomLatitude, float leftLongitude, float rightLongitude) {
    this(0.0, 0.0, mapScreenWidth, mapScreenHeight, topLatitude, bottomLatitude, leftLongitude, rightLongitude);
  }
  
  /**
   * Creates a new MercatorMap with dimensions and bounding box to convert between geo-locations and screen coordinates.
   *
   * @param mapScreenOffsetX Horizontal offset of this map, in pixels. 
   * @param mapScreenOffsetY Vertical offset of this map, in pixels. 
   ...
   */
  public MercatorMap(float mapScreenOffsetX, float mapScreenOffsetY, float mapScreenWidth, float mapScreenHeight, float topLatitude, float bottomLatitude, float leftLongitude, float rightLongitude) {
    this.mapScreenOffsetX = mapScreenOffsetX;
    this.mapScreenOffsetY = mapScreenOffsetY;
  ...
  protected float getScreenY(float latitudeInDegrees) {
    return mapScreenOffsetY + mapScreenHeight * (getScreenYRelative(latitudeInDegrees) - topLatitudeRelative) / (bottomLatitudeRelative - topLatitudeRelative);
  }
  ...
  protected float getScreenX(float longitudeInDegrees) {
    ...
    return mapScreenOffsetX + mapScreenWidth * (longitudeInRadians - leftLongitudeRadians) / (rightLongitudeRadians - leftLongitudeRadians);
  }
}
